...
| api/questions/ | post | Создание вопроса к определенному опросу
| api/questions/:id | patch | Обновление данных вопроса
| api/questions/:p_id/:q_id/answers/ | post | Отправка ответа пользователя на вопрос опроса
...
Ссылка: ```api/questions/:p_id/:q_id/answers```
Метод: ```post```
*__Доступно для ВСЕХ пользователей.__*
В URL передается p_id - ID ОПРОСА и q_id - ID ВОПРОСА
В случае передачи не существующего ID ОПРОСА, ВОПРОСА или ВАРИНТА ОТВЕТА вернется HTTP_404_NOT_FOUND. Ответ записан НЕ БУДЕТ.
На вход принимает json вида:
```json
{"answers": {
