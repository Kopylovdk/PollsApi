...
| api/users/answers/ | post | Отправка ответа пользователя на вопрос опроса
| api/users/answers/ | get | Получение пройденных опросов пользователя
| api/polls/create/ | post | Создание опроса
| api/polls/update/:id | patch | Изменение опроса
| api/polls/list/ | get | Получение списка ВСЕХ опросов
| api/polls/ | get | Получение списка АКТИВНЫХ опросов
...
| api/questionOptions/:id | post | Создание вариантов ответа на определенный вопрос
| api/questionOptions/:id | patch | Обновление данных варианта ответа на вопрос
...
В случае попытки получить неактивный опрос вернется статус 403_FORBIDDEN.
...
При успешном выполнении запроса вернется статус 201_CREATED и json c созданными данными:
