...
``` 
#### Возможно запустить файл run.bat - В результате будут проделаны все действия, указанные ниже (включая удаление существующей БД).
```
папка проекта/run.bat
```
#### Создание миграций, применение миграций.
```
...
```
#### Для изменения secret_key в файле main.settings необходимо изменить значение SECRET_KEY  
```
SECRET_KEY = 'Ваш ключ'
```
#### Запускаем сервер
